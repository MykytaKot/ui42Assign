            ...
            <div class="h-fit-content">
                ...
                <form action="" class="container align-center">
                    <div class="input-group w-75 shadow  ">
                        <input type="text" id="search" class="form-control rounded-1 p-3 pl-4 pr-4" v-model="searchTerm"
                            placeholder="Zadajte nazov ..." aria-label="Search">
                ...
                <div v-if="searchResults.length > 0" class="results-container bg-white mt-2 mb-4 shadow  px-4 rounded">
                    <div v-for="result in searchResults.slice(0, 10)" :key="result.id" class="w-100">
                        ...
                        <router-link :to="'/city/' + result.id"
                            class="btn jumbotron border-top link-primary d-flex row"><img
                                class="col-md-1 col-sm-2 d-none d-md-block  d-lg-block d-xl-block "
                                :src="result.img_path" alt="">
                            <h5 class="col-md-5 col-sm-4 text-start">{{ result.name }} </h5> <span class="col-6">
                                Región:{{ result.region }} </span>
                        </router-link>
